...
const styles = (req, res, includeSheets = [], excludeSheets = []) => {
    let sarr = [];
    const theme = op.get(req, 'query.theme', process.env.DEFAULT_THEME || 'style');
    ...
    if (isToolkit(req.path)) {
        ...
        styles.push(`<link rel="stylesheet" href="${p.split(publicDir).join('')}">`);
    } else {
        styles = styles.concat(
            ...
            .map(item => {
                ...
                if (fs.statSync(p).isFile()) {
                    return `<link rel="stylesheet" href="${p.split(publicDir).join('')}">`;
                }
                return false;
            })
            .filter(item => {
                if (item && includes.find(search => item.indexOf(search) >= 0)) {
                    return true;
                }

                if (!item || excludes.find(search => item.indexOf(search) >= 0)) {
                    return false;
                }
            })
        );
